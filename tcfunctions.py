...
def torus_error(m_stablz, m_qubits, p):
	...
	Input:
	  ...
	  error probability p
	Output:
	  stabilizer and qubits matrix with random ganerated errors.
	"""
	values = [0,1]
	prob = [1-p, p]
	ly = len(m_stablz)
...
def correct_qubits(matching,er_qubits):
	...
	for i in range(len(matching_list)):
	    ...
	    if abs(aux_b-aux_d) <= int(lx/2):
	        for j in range(min(aux_b,aux_d),max(aux_b,aux_d)):
	            crted_qubits[q_row][j][0] += 1
	    else:
	        for j in range(0, min(aux_b,aux_d)):
	            crted_qubits[q_row][j][0] += 1
	        for j in range(max(aux_b,aux_d), lx):
	            crted_qubits[q_row][j][0] += 1
	    if aux_a >= aux_c:
	        q_col = aux_b
	    else:
	    ...
	    if abs(aux_a-aux_c) <= int(ly/2):
	        for j in range(2*min(aux_a,aux_c)+1,2*max(aux_a,aux_c)+1,2):
	            crted_qubits[j][q_col][0] += 1
	    else:
	        for j in range(1, 2*min(aux_a,aux_c)+1,2):
	            crted_qubits[j][q_col][0] += 1
	        for j in range(2*max(aux_a,aux_c)+1, 2*ly+1,2):
	            crted_qubits[j][q_col][0] += 1
	for i_row in range(2*ly):
	    for i_col in range(lx):
...
def check_logical(crted_qubits):
	...
	n_error_row = crted_qubits[1].count([1])
	if n_error_row % 2 == 1:
